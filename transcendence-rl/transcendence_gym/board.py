...
from transcendence_gym.constants import (
  ...
  DistortModeType, BlockType, DistortApplicapableEffects
)
...
class Board(object):
  def __init__(self, board_values:np.ndarray=None):
    ...
    if not (board_values is None):
      ...
      self.values = board_values
       ...
       self.values = np.full((MAX_BOARD_SIZE, MAX_BOARD_SIZE), BlockType.NORMAL)
  ...
  def _removeBlock(self, x, y, distort_mode:str = "default"):
    ...
    if not (0 <= x < len(self.values) and 0 <= y < len(self.values[0])):
        return -1, -1
    if self.values[x][y] == BlockType.EMPTY or self.values[x][y] == BlockType.DESTROYED:
        return -1, -1

    # Try to remove the block
    if self.values[x][y] == BlockType.DISTORTED:
        if distort_mode == DistortModeType.BREAK:
            self.values[x][y] = BlockType.DESTROYED
            return x, y
            ...
            positions = np.transpose(np.nonzero(self.values == BlockType.DESTROYED))
            ...
            for pos in new_blocks_pos:
                self.values[pos[0]][pos[1]] = BlockType.NORMAL
    elif self.values[x][y] == BlockType.NORMAL:
        self.values[x][y] = BlockType.DESTROYED
        return x, y
    else:
  ...
  def applyEffect(self, effect, x, y):
    ...
    if self.values[x][y] == BlockType.DISTORTED and not distort_applicapable:
